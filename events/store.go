...
import (
	"encoding/json"
	"github.com/cesc1802/core-service/plugin/storage/sdkmemory"
	"time"
...
func NewStore(opts ...StoreOption) Store {
	...
	evs := &evStore{
		opts:  options,
		store: sdkmemory.NewMemoryStore(),
	}
...
func (s *evStore) Read(topic string, opts ...ReadOption) ([]*Event, error) {
	...
	recs, err := s.store.Read(topic+joinKey,
		sdkmemory.ReadPrefix(),
		sdkmemory.ReadLimit(options.Limit),
		sdkmemory.ReadOffset(options.Offset),
	)
	if err != nil {
...
type Backup interface {
	Snapshot(st sdkmemory.Store) error
}
